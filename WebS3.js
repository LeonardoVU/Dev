...
(async function () {
    ...
    //Maximo en 1 linea 15 caracteres
    ...
    const createScene = async function () {
        ...
        anchor.position.y = 5;
        anchor.position.z = 25;

        var stxt = "";
        var addButton = function (txt, value, pos) {
            //console.log(pos);
            ...
            if (typeof value === "object") {
                ...
                var myText = BABYLON.MeshBuilder.CreateText(txt, otxt, fontData, {
                    size: 0.5,
                    resolution: 128,
                    depth: 0.1
                });
                ...
                myText.position.y = 5;// * (en.length + 1);//125;
                myText.position.z = 5;// * (en.length + 1);//-250;
                myText.position.x = 5 * pos;//-250;
        ...
        scene.registerBeforeRender(() => {
            ...
            if (clk === -1) {
                ...
                //panel._columns = 24;
                for (let key in e) {
                    let ord = Object.keys(e).indexOf(key);
                    //console.log(ord, key);

                    //console.log(mn[key].lbl);
                    addButton(key, e[key], ord);
                }
            ...
            if (clk === 1) {
                ...
                //console.log(mstxt[mstxt.length-1]);
/*                 scene.meshes.forEach(itm => {
                    if(itm.id.slice(-7)==="_merged"){
                        //console.log(itm.id);
                        scene.removeMesh(itm);
                    }

                    //let msh = scene.getMeshByName(itm);
                    //scene.removeMesh(msh);
                }); */
        ...
        scene.onPointerObservable.add((pointerInfo) => {
            switch (pointerInfo.type) {
                ...
                case BABYLON.PointerEventTypes.POINTERPICK:
                    ...
                    try {
                        stxt = pointerInfo.pickInfo.pickedMesh.name.replace(/\n/g, "_");
                    }
                    catch {
                        stxt = pointerInfo.pickInfo.pickedMesh.name;// = txt;
                    }
                    //stxt = text1.text;
                    clk = 1;
                    ...
                    switch (kbInfo.event.key) {
                        ...
                        case "t":
                            ...
                            async function getFile() {
                                ...
                                console.log([fileHandle][0].name);
                                console.log(fileHandle);

                                /* const importPromise = BABYLON.SceneLoader.ImportMeshAsync("", "./assets/Models/", [fileHandle][0].name, scene);

                                importPromise.then((result) => {
                                    //// Result has meshes, particleSystems, skeletons, animationGroups and transformNodes
                                    console.log(result);

                                    result.meshes.forEach((itm) => {
                                        //itm.convertToUnIndexedMesh();
                                        //itm.parent = CoT;
                                        itm.rotation.x = -Math.PI / 2;
                                        itm.scaling = new BABYLON.Vector3(0.1, 0.1, 0.1);
                                        console.log(itm.getBoundingInfo().boundingBox.center);
                                    });
    ...
    window.addEventListener("resize", function () {
        engine.resize();
        //console.log(document.getElementById("get-files").value);
